...
void CLOCK_DMA_Start(void) 
{
    ...
    CLOCK_DMA_CLKEN |= CLOCK_DMA_CLKEN_MASK;
	CLOCK_DMA_CLKSTBY |= CLOCK_DMA_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: CLOCK_DMA_Stop
********************************************************************************
*
* Summary:
...
void CLOCK_DMA_Stop(void) 
{
    ...
    CLOCK_DMA_CLKEN &= (uint8)(~CLOCK_DMA_CLKEN_MASK);
	CLOCK_DMA_CLKSTBY &= (uint8)(~CLOCK_DMA_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: CLOCK_DMA_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((CLOCK_DMA_CLKEN & CLOCK_DMA_CLKEN_MASK) != 0u)
    {
...
#else
        CLK_DIST_DMASK = CLOCK_DMA_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(CLOCK_DMA_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(CLOCK_DMA_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: CLOCK_DMA_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: CLOCK_DMA_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = CLOCK_DMA_GetSourceRegister();
    uint16 oldDivider = CLOCK_DMA_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = CLOCK_DMA_CLKEN & CLOCK_DMA_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((CLOCK_DMA_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: CLOCK_DMA_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(CLOCK_DMA_DIV_PTR);
}


/*******************************************************************************
* Function Name: CLOCK_DMA_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    CLOCK_DMA_MOD_SRC |= modeBitMask & (uint8)CLOCK_DMA_MODE_MASK;
}


/*******************************************************************************
* Function Name: CLOCK_DMA_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    CLOCK_DMA_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(CLOCK_DMA_MODE_MASK));
}


/*******************************************************************************
* Function Name: CLOCK_DMA_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return CLOCK_DMA_MOD_SRC & (uint8)(CLOCK_DMA_MODE_MASK);
}


/*******************************************************************************
* Function Name: CLOCK_DMA_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = CLOCK_DMA_GetDividerRegister();
    uint8 oldSrc = CLOCK_DMA_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        CLOCK_DMA_MOD_SRC |= CYCLK_SSS;
        CLOCK_DMA_MOD_SRC =
            (CLOCK_DMA_MOD_SRC & (uint8)(~CLOCK_DMA_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: CLOCK_DMA_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return CLOCK_DMA_MOD_SRC & CLOCK_DMA_SRC_SEL_MSK;
}
