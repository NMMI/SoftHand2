...
/*******************************************************************************
* Function Name: ADC_COUNTER_CLK_1_Start
********************************************************************************
*
* Summary:
...
void ADC_COUNTER_CLK_1_Start(void) 
{
    ...
    ADC_COUNTER_CLK_1_CLKEN |= ADC_COUNTER_CLK_1_CLKEN_MASK;
	ADC_COUNTER_CLK_1_CLKSTBY |= ADC_COUNTER_CLK_1_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: ADC_COUNTER_CLK_1_Stop
********************************************************************************
*
* Summary:
...
void ADC_COUNTER_CLK_1_Stop(void) 
{
    ...
    ADC_COUNTER_CLK_1_CLKEN &= (uint8)(~ADC_COUNTER_CLK_1_CLKEN_MASK);
	ADC_COUNTER_CLK_1_CLKSTBY &= (uint8)(~ADC_COUNTER_CLK_1_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: ADC_COUNTER_CLK_1_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((ADC_COUNTER_CLK_1_CLKEN & ADC_COUNTER_CLK_1_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = ADC_COUNTER_CLK_1_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = ADC_COUNTER_CLK_1_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* ADC_COUNTER_CLK_1__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(ADC_COUNTER_CLK_1_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(ADC_COUNTER_CLK_1_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: ADC_COUNTER_CLK_1_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: ADC_COUNTER_CLK_1_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = ADC_COUNTER_CLK_1_GetSourceRegister();
    uint16 oldDivider = ADC_COUNTER_CLK_1_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = ADC_COUNTER_CLK_1_CLKEN & ADC_COUNTER_CLK_1_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(ADC_COUNTER_CLK_1_DIV_PTR, clkDivider);
                ADC_COUNTER_CLK_1_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                ADC_COUNTER_CLK_1_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(ADC_COUNTER_CLK_1_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((ADC_COUNTER_CLK_1_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(ADC_COUNTER_CLK_1_DIV_PTR, clkDivider);
				ADC_COUNTER_CLK_1_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: ADC_COUNTER_CLK_1_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(ADC_COUNTER_CLK_1_DIV_PTR);
}


/*******************************************************************************
* Function Name: ADC_COUNTER_CLK_1_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    ADC_COUNTER_CLK_1_MOD_SRC |= modeBitMask & (uint8)ADC_COUNTER_CLK_1_MODE_MASK;
}


/*******************************************************************************
* Function Name: ADC_COUNTER_CLK_1_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    ADC_COUNTER_CLK_1_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(ADC_COUNTER_CLK_1_MODE_MASK));
}


/*******************************************************************************
* Function Name: ADC_COUNTER_CLK_1_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return ADC_COUNTER_CLK_1_MOD_SRC & (uint8)(ADC_COUNTER_CLK_1_MODE_MASK);
}


/*******************************************************************************
* Function Name: ADC_COUNTER_CLK_1_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = ADC_COUNTER_CLK_1_GetDividerRegister();
    uint8 oldSrc = ADC_COUNTER_CLK_1_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        ADC_COUNTER_CLK_1_MOD_SRC |= CYCLK_SSS;
        ADC_COUNTER_CLK_1_MOD_SRC =
            (ADC_COUNTER_CLK_1_MOD_SRC & (uint8)(~ADC_COUNTER_CLK_1_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: ADC_COUNTER_CLK_1_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return ADC_COUNTER_CLK_1_MOD_SRC & ADC_COUNTER_CLK_1_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: ADC_COUNTER_CLK_1_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    ADC_COUNTER_CLK_1_PHASE = clkPhase & ADC_COUNTER_CLK_1_PHASE_MASK;
}


/*******************************************************************************
* Function Name: ADC_COUNTER_CLK_1_GetPhase
********************************************************************************
*
* Summary:
...
{
    return ADC_COUNTER_CLK_1_PHASE & ADC_COUNTER_CLK_1_PHASE_MASK;
}
