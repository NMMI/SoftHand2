...
/*******************************************************************************
* Function Name: ADC_SOC_CLK_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ADC_SOC_CLK_Start(void) 
{
    /* Set the bit to enable the clock. */
    ADC_SOC_CLK_CLKEN |= ADC_SOC_CLK_CLKEN_MASK;
	ADC_SOC_CLK_CLKSTBY |= ADC_SOC_CLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: ADC_SOC_CLK_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ADC_SOC_CLK_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    ADC_SOC_CLK_CLKEN &= (uint8)(~ADC_SOC_CLK_CLKEN_MASK);
	ADC_SOC_CLK_CLKSTBY &= (uint8)(~ADC_SOC_CLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: ADC_SOC_CLK_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ADC_SOC_CLK_StopBlock(void) 
{
    if ((ADC_SOC_CLK_CLKEN & ADC_SOC_CLK_CLKEN_MASK) != 0u)
    {
...
#if defined(ADC_SOC_CLK__CFG3)
        CLK_DIST_AMASK = ADC_SOC_CLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = ADC_SOC_CLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* ADC_SOC_CLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(ADC_SOC_CLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(ADC_SOC_CLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: ADC_SOC_CLK_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ADC_SOC_CLK_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        ADC_SOC_CLK_CLKSTBY &= (uint8)(~ADC_SOC_CLK_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: ADC_SOC_CLK_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ADC_SOC_CLK_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = ADC_SOC_CLK_GetSourceRegister();
    uint16 oldDivider = ADC_SOC_CLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = ADC_SOC_CLK_CLKEN & ADC_SOC_CLK_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((ADC_SOC_CLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: ADC_SOC_CLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 ADC_SOC_CLK_GetDividerRegister(void) 
{
    return CY_GET_REG16(ADC_SOC_CLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: ADC_SOC_CLK_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ADC_SOC_CLK_SetModeRegister(uint8 modeBitMask) 
{
    ADC_SOC_CLK_MOD_SRC |= modeBitMask & (uint8)ADC_SOC_CLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: ADC_SOC_CLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ADC_SOC_CLK_ClearModeRegister(uint8 modeBitMask) 
{
    ADC_SOC_CLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(ADC_SOC_CLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: ADC_SOC_CLK_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 ADC_SOC_CLK_GetModeRegister(void) 
{
    return ADC_SOC_CLK_MOD_SRC & (uint8)(ADC_SOC_CLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: ADC_SOC_CLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ADC_SOC_CLK_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = ADC_SOC_CLK_GetDividerRegister();
    uint8 oldSrc = ADC_SOC_CLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        ADC_SOC_CLK_MOD_SRC |= CYCLK_SSS;
        ADC_SOC_CLK_MOD_SRC =
            (ADC_SOC_CLK_MOD_SRC & (uint8)(~ADC_SOC_CLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: ADC_SOC_CLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 ADC_SOC_CLK_GetSourceRegister(void) 
{
    return ADC_SOC_CLK_MOD_SRC & ADC_SOC_CLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: ADC_SOC_CLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ADC_SOC_CLK_SetPhaseRegister(uint8 clkPhase) 
{
    ADC_SOC_CLK_PHASE = clkPhase & ADC_SOC_CLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: ADC_SOC_CLK_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 ADC_SOC_CLK_GetPhaseRegister(void) 
{
    return ADC_SOC_CLK_PHASE & ADC_SOC_CLK_PHASE_MASK;
}
