/*******************************************************************************
* File Name: Signal_2_B.h  
* Version 2.20
*
* Description:
*  This file contains Pin function prototypes and register defines
*
* Note:
*
********************************************************************************
* Copyright 2008-2015, Cypress Semiconductor Corporation.  All rights reserved.
* You may use this file only in accordance with the license, terms, conditions, 
* disclaimers, and limitations in the end user license agreement accompanying 
* the software package with which this file was provided.
*******************************************************************************/

#if !defined(CY_PINS_Signal_2_B_H) /* Pins Signal_2_B_H */
#define CY_PINS_Signal_2_B_H

#include "cytypes.h"
#include "cyfitter.h"
#include "cypins.h"
#include "Signal_2_B_aliases.h"


/***************************************
*        Function Prototypes             
***************************************/    

/**
* \addtogroup group_general
* @{
*/
void    Signal_2_B_Write(uint8 value) ;
void    Signal_2_B_SetDriveMode(uint8 mode) ;
uint8   Signal_2_B_ReadDataReg(void) ;
uint8   Signal_2_B_Read(void) ;
void    Signal_2_B_SetInterruptMode(uint16 position, uint16 mode) ;
uint8   Signal_2_B_ClearInterrupt(void) ;
/** @} general */

/***************************************
*           API Constants        
***************************************/

/**
* \addtogroup group_constants
* @{
*/
    /** \addtogroup driveMode Drive mode constants
     * \brief Constants to be passed as "mode" parameter in the Signal_2_B_SetDriveMode() function.
     *  @{
     */
        /* Drive Modes */
        #define Signal_2_B_DM_ALG_HIZ         PIN_DM_ALG_HIZ   /**< \brief High Impedance Analog   */
        #define Signal_2_B_DM_DIG_HIZ         PIN_DM_DIG_HIZ   /**< \brief High Impedance Digital  */
        #define Signal_2_B_DM_RES_UP          PIN_DM_RES_UP    /**< \brief Resistive Pull Up       */
        #define Signal_2_B_DM_RES_DWN         PIN_DM_RES_DWN   /**< \brief Resistive Pull Down     */
        #define Signal_2_B_DM_OD_LO           PIN_DM_OD_LO     /**< \brief Open Drain, Drives Low  */
        #define Signal_2_B_DM_OD_HI           PIN_DM_OD_HI     /**< \brief Open Drain, Drives High */
        #define Signal_2_B_DM_STRONG          PIN_DM_STRONG    /**< \brief Strong Drive            */
        #define Signal_2_B_DM_RES_UPDWN       PIN_DM_RES_UPDWN /**< \brief Resistive Pull Up/Down  */
    /** @} driveMode */
/** @} group_constants */
    
/* Digital Port Constants */
#define Signal_2_B_MASK               Signal_2_B__MASK
#define Signal_2_B_SHIFT              Signal_2_B__SHIFT
#define Signal_2_B_WIDTH              1u

/* Interrupt constants */
#if defined(Signal_2_B__INTSTAT)
/**
* \addtogroup group_constants
* @{
*/
    /** \addtogroup intrMode Interrupt constants
     * \brief Constants to be passed as "mode" parameter in Signal_2_B_SetInterruptMode() function.
     *  @{
     */
        #define Signal_2_B_INTR_NONE      (uint16)(0x0000u)   /**< \brief Disabled             */
        #define Signal_2_B_INTR_RISING    (uint16)(0x0001u)   /**< \brief Rising edge trigger  */
        #define Signal_2_B_INTR_FALLING   (uint16)(0x0002u)   /**< \brief Falling edge trigger */
        #define Signal_2_B_INTR_BOTH      (uint16)(0x0003u)   /**< \brief Both edge trigger    */
        /** @} intrMode */
/** @} group_constants */

    #define Signal_2_B_INTR_MASK      (0x01u)
#endif /* (Signal_2_B__INTSTAT) */


/***************************************
*             Registers        
***************************************/

/* Main Port Registers */
/* Pin State */
#define Signal_2_B_PS                     (* (reg8 *) Signal_2_B__PS)
/* Data Register */
#define Signal_2_B_DR                     (* (reg8 *) Signal_2_B__DR)
/* Port Number */
#define Signal_2_B_PRT_NUM                (* (reg8 *) Signal_2_B__PRT) 
/* Connect to Analog Globals */                                                  
#define Signal_2_B_AG                     (* (reg8 *) Signal_2_B__AG)                       
/* Analog MUX bux enable */
#define Signal_2_B_AMUX                   (* (reg8 *) Signal_2_B__AMUX) 
/* Bidirectional Enable */                                                        
#define Signal_2_B_BIE                    (* (reg8 *) Signal_2_B__BIE)
/* Bit-mask for Aliased Register Access */
#define Signal_2_B_BIT_MASK               (* (reg8 *) Signal_2_B__BIT_MASK)
/* Bypass Enable */
#define Signal_2_B_BYP                    (* (reg8 *) Signal_2_B__BYP)
/* Port wide control signals */                                                   
#define Signal_2_B_CTL                    (* (reg8 *) Signal_2_B__CTL)
/* Drive Modes */
#define Signal_2_B_DM0                    (* (reg8 *) Signal_2_B__DM0) 
#define Signal_2_B_DM1                    (* (reg8 *) Signal_2_B__DM1)
#define Signal_2_B_DM2                    (* (reg8 *) Signal_2_B__DM2) 
/* Input Buffer Disable Override */
#define Signal_2_B_INP_DIS                (* (reg8 *) Signal_2_B__INP_DIS)
/* LCD Common or Segment Drive */
#define Signal_2_B_LCD_COM_SEG            (* (reg8 *) Signal_2_B__LCD_COM_SEG)
/* Enable Segment LCD */
#define Signal_2_B_LCD_EN                 (* (reg8 *) Signal_2_B__LCD_EN)
/* Slew Rate Control */
#define Signal_2_B_SLW                    (* (reg8 *) Signal_2_B__SLW)

/* DSI Port Registers */
/* Global DSI Select Register */
#define Signal_2_B_PRTDSI__CAPS_SEL       (* (reg8 *) Signal_2_B__PRTDSI__CAPS_SEL) 
/* Double Sync Enable */
#define Signal_2_B_PRTDSI__DBL_SYNC_IN    (* (reg8 *) Signal_2_B__PRTDSI__DBL_SYNC_IN) 
/* Output Enable Select Drive Strength */
#define Signal_2_B_PRTDSI__OE_SEL0        (* (reg8 *) Signal_2_B__PRTDSI__OE_SEL0) 
#define Signal_2_B_PRTDSI__OE_SEL1        (* (reg8 *) Signal_2_B__PRTDSI__OE_SEL1) 
/* Port Pin Output Select Registers */
#define Signal_2_B_PRTDSI__OUT_SEL0       (* (reg8 *) Signal_2_B__PRTDSI__OUT_SEL0) 
#define Signal_2_B_PRTDSI__OUT_SEL1       (* (reg8 *) Signal_2_B__PRTDSI__OUT_SEL1) 
/* Sync Output Enable Registers */
#define Signal_2_B_PRTDSI__SYNC_OUT       (* (reg8 *) Signal_2_B__PRTDSI__SYNC_OUT) 

/* SIO registers */
#if defined(Signal_2_B__SIO_CFG)
    #define Signal_2_B_SIO_HYST_EN        (* (reg8 *) Signal_2_B__SIO_HYST_EN)
    #define Signal_2_B_SIO_REG_HIFREQ     (* (reg8 *) Signal_2_B__SIO_REG_HIFREQ)
    #define Signal_2_B_SIO_CFG            (* (reg8 *) Signal_2_B__SIO_CFG)
    #define Signal_2_B_SIO_DIFF           (* (reg8 *) Signal_2_B__SIO_DIFF)
#endif /* (Signal_2_B__SIO_CFG) */

/* Interrupt Registers */
#if defined(Signal_2_B__INTSTAT)
    #define Signal_2_B_INTSTAT             (* (reg8 *) Signal_2_B__INTSTAT)
    #define Signal_2_B_SNAP                (* (reg8 *) Signal_2_B__SNAP)
    
	#define Signal_2_B_0_INTTYPE_REG 		(* (reg8 *) Signal_2_B__0__INTTYPE)
#endif /* (Signal_2_B__INTSTAT) */

#endif /* End Pins Signal_2_B_H */


/* [] END OF FILE */
